...
namespace SaleProducts.Api.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        {
            var productDto = await _productService.Handle(command);
        ...
        {
            var productDto = await _productService.Handle(new GetProductByIdQuery(id));
            if (productDto == null)
            {
                return NotFound();
            }
        ...
        {
            var products = await _productService.Handle(new GetAllProductsQuery());
            return Ok(products);
        }
        ...
        public async Task<IActionResult> DeleteProduct(Guid id)
        {
            try
            {
                await _productService.Handle(new DeleteProductCommand(id));
                return NoContent();
            }
            catch (KeyNotFoundException ex)
            {

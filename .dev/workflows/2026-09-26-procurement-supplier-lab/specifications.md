...
Receipt creation records UTC time at millisecond precision. The first HTTP result, persisted receipt, replay and source-event payload must retain exactly the same ReceivedAt value. This precision clarification follows the actual PostgreSQL test: .NET sub-microsecond ticks were otherwise lost on persistence. It does not change receipt identity or delivery ordering.
...
Fixed POST responses require the complete top-level fixture identity (clientRequestId, SKU, quantity, unitPrice, currency) in both mock and hybrid modes. Equivalent numeric prices such as 100 and 100.00 match; changed fields or nested decoys do not. WireMock uses its native JMESPath matcher, and Microcks uses its native JS dispatcher. Mode names are case-insensitive named values only, never numeric enum aliases. Reset is serialized: before mutation caller cancellation is respected; after mutation a separate bounded token completes the native rebuild. State is applying/ready/failed; effective mode is null outside ready and health is unavailable. A failed reset is visible and a subsequent successful reset can recover it. Mapping replacement is not claimed to be request-atomic during the applying interval.

#!/usr/bin/env python3
"""Finalize the independently audited v0.11 target authorities and rule state."""
...
REPORT = (
    ...
    "reports/05-v0.11.0-reconciliation.md"
)
PREFLIGHT = (
    ".dev/workflows/2026-08-12-ai-context-v0-13-upgrade/"
    "evidence/v0.11.0/preflight.yaml"
)
SUMMARY = (
    ".dev/workflows/2026-08-12-ai-context-v0-13-upgrade/"
    "evidence/v0.11.0/validation-summary.yaml"
)
ASSESSMENT = ".dev/assessments/ASM-20260813-004/report.md"
EXPECTED = {
    PROVENANCE: "b6e7f4452ac973e71f3859b4a41e2b852b81545954ed7d9f4823ee6a0290f5ea",
    LEDGER: "81b179b585c47d55d5ecf9d0614291927513904d0570029715693861a1bb9ced",
    RECEIPT: "cfabfcfbc7f76556f036abbbf145c3088d1ddad64b96288d084c6b3b751e9ebf",
}
EXPECTED_IDS = {
...
def main() -> int:
    ...
    if provenance.get("source", {}).get("version") != "v0.10.0":
        raise RuntimeError("v0.11 finalization requires exact finalized v0.10 authority")
    ...
    if previous_source != expected_previous:
        raise RuntimeError(f"unexpected v0.10 source authority: {previous_source}")
    ...
    by_id["CUST-DOTNET-MQ-GOVERNANCE"]["reason"] = (
        "Merge the byte-exact v0.11 product-source projection contract while "
        "retaining target Issue-first authorization, repository governance and "
        "skill routing, directory-scoped .codex/agents tracking, the repository "
        "LF policy, and inactive provider/profile/reuse boundaries."
    )
    by_id["CUST-DOTNET-MQ-VALIDATION"]["reason"] = (
        "AICU-V010-PROJECTION-001, carried AICU-V090-DOC-001, and "
        "AICU-V011-SELECTION-001 prove that the v0.11 stock downstream "
        "projection and changed-path dependency selection cannot be an "
        "authoritative target gate. Use the SHA-pinned target gate without "
        "claiming stock profiles, reuse, or check-all passed."
    )
    by_id["CUST-DOTNET-MQ-REPO-TRUTH"]["reason"] = (
        "Preserve repository identity, SDK and product truth, target catalogs "
        "and workflow authorities, complete analyzer/tools retirement, and the "
        "inactive provider boundary. The v0.11 product-source contract affirms "
        "that these target facts remain outside framework package authority."
    )
    by_id["CUST-DOTNET-MQ-EXECUTION-PROVENANCE-ADOPTION"]["reason"] = (
        ...
        "fail-closed overlay while canonical v0.11 framework bytes remain exact."
    )

...
- Authorized remediation scope: preserve the full downstream feedback under `docs/`, remove confirmed source-only requirement leaks, remove source workflow backlinks, correct active architecture path casing, and add regression validation.
...
- Externalized feedback: delivered to the user and intentionally removed from the repository before commit.

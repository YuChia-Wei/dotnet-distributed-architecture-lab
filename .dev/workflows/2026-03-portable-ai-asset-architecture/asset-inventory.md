...
> Historical snapshot: 本文件記錄的是重構初期的資產盤點，部分內容已被後續的 canonical skill / sub-agent-role 模型取代。使用目前狀態時，應以 `workflow-plan.md`、`prompt-retention-checklist.md`、`SKILL-AND-SUB-AGENT-TAXONOMY-GUIDE.md` 與最新 task / handoff 記錄為準。
...
| `.dev/workflows/` | workflow artifacts | 保存規劃與執行中的 plan / task / review |

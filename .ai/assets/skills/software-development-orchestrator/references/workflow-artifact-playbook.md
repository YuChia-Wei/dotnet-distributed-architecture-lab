...
Create or switch to `codex/<workflow-id>` or the active runtime equivalent before creating these artifacts. Record `branch` and `base_branch` in the locator and plan. Follow `.dev/TEAM-GIT-FLOW-RULES.MD` for branch naming, push handoff, checkpoint merge, continuation, and merge strategy. A merge/push before completion keeps the plan and pending tasks active but `in_progress` is not itself an integration blocker: after separate authorization, a valid checkpoint, and passing repository gates, resume push-only handoffs from the pushed branch and create a continuation branch from the updated target after merge.
